...
exports.get_my_reply = function (req, res) {
    var attr = ' as table_name, rid, contents, bid, attorney_info.aid, name, profile_image_src, date_format(reply_';
    var union_query = so.get_union_query(req, 'reply', attr);
    db.SELECT('"administration"' + attr + 'administration.date, "%Y-%m-%d %H:%i:%S") AS date', 'reply_administration, attorney_info', union_query).then((data) => { 
