...
exports.get_post_and_reply = function (res, category, bid) {
    ...
    var attr = 'attorney_info.aid, name, profile_image_src, ' + reply_table + '.contents, ' + reply_table + '.date';
...
exports.get_attorney_detail = function(res, aid) {
    db.SELECT('*', 'attorney_info', 'WHERE aid=' + aid).then((data) => {
            res.json(data[0]);
    })
}
