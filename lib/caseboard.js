...
exports.get_cases = function (res, category, offset, order) {
    ...
    db.SELECT('COUNT(*) AS cnt', 'case_board', 'WHERE category="' + category + '" ').then((data) => { //then : resolve 호출된 경우
        ...
        cn = JSON.parse(JSON.stringify(data[0]));
        db.SELECT('cid, title, image_src, name, attorney_info.aid, clicks', 'case_board, attorney_info', where_statement + order_statement + limit_statement).then((data) => { //then : resolve 호출된 경우
            cases = JSON.parse(JSON.stringify(data));
            result = {count : cn.cnt, cases: cases}
            console.log(result);
...
exports.get_case_contents = function (cid, res) {
    db.UPDATE('clicks = clicks + 1', 'case_board', 'WHERE cid=' + cid).then((data => { //조회수 + 1  
